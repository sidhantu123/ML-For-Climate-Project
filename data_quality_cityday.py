...
aqi_missing = city_day_df.groupby('City')['AQI'].apply(lambda x: x.isna().sum()).reset_index()
...
aqi_stats['missing_percentage'] = (aqi_stats['AQI'] / aqi_stats['total_records'] * 100).round(2)
...
print(f"\nOverall Statistics for city_day.csv:")
...
print("================================")
time_range = city_day_df.groupby('City').agg({
    'Date': ['min', 'max']
}).reset_index()
