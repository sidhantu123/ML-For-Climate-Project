...
# Function to plot MAE for selected models for all cities on a single graph
def plot_selected_models():
    ...
    ax.set_ylabel("MAE")
    ax.set_title("MAE for Selected Models Across Cities")
    ax.set_xticks([i + width * 2 for i in x])
    ax.set_xticklabels(selected_models, rotation=0, ha="center")
    ax.legend()
    plt.tight_layout()
    plt.savefig("../notebooks/results/plots/selected_models_mae_all_cities.png")
    plt.close()
